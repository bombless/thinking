...
class Model:
    ...
    thinking_network = ThinkingNetwork(embedding_layer)
    generating_network = GeneratingNetwork(embedding_layer)

    def get_thinking_input(self, emb):
        pad = tok.encode("<pad>")[0]
        pad = self.embedding_layer(torch.tensor(pad)).unsqueeze(0).unsqueeze(0)
    ...
    def forward_thinking(self, network: ThinkingNetwork, tokens_data, mask=None):
        ...
        return network(rope_result, mask)
    ...
    def forward_generating(self, thinking_network: ThinkingNetwork, generating_network: GeneratingNetwork, tokens_data, mask=None):
        ...
        thinking_result = thinking_network(prompt_rope_result)
    ...
    def get_thinking_training_data(self):
        ...
        tensor_thinking = torch.tensor(thinking)
        print(tensor_thinking)
        print('vocab_size ' + str(vocab_size))
        return torch.tensor(prompt).unsqueeze(0), self.embedding_layer(tensor_thinking).unsqueeze(0)
